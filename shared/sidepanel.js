...
document.addEventListener('DOMContentLoaded', () => {
  ...
  chrome.windows.getCurrent((win) => {
    if (win) {
      ...
      // Initialize active tab ID synchronously on load in current window
      chrome.tabs.query({ active: true, windowId: currentWindowId }, ([tab]) => {
        if (tab) {
  ...
  // Keep track of active tab ID on tab switches in the CURRENT window
  ...
  // Keep track of active tab on window focus changes (only if it matches our window)
  chrome.windows.onFocusChanged.addListener((windowId) => {
    if (windowId === chrome.windows.WINDOW_ID_NONE || windowId !== currentWindowId) return;
    chrome.tabs.query({ active: true, windowId: currentWindowId }, ([tab]) => {
      if (tab) {
        currentActiveTabId = tab.id;
  ...
  // Keep track of active tab on tab reloads/updates in the CURRENT window
  chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (tab.windowId === currentWindowId && tab.active) {
      currentActiveTabId = tabId;
      console.log('🎙️ [Sidepanel] Active tab changed (updated):', currentActiveTabId);
      syncLiveStatus();
    }
  });
...
function bindListeners() {
  ...
  document.getElementById('ltTtsToggle').addEventListener('change', async () => {
    ...
    if (!enabled) {
      // See popup.js: stop() alone drains the background queue rather than
      // ending it, because 'interrupted' advances to the next item.
...
async function syncLiveStatus(retries = 3) {
  try {
    const result = await chrome.storage.local.get(['ltSourceLang', 'ltTgtLang', 'ltMode', 'ltEngine', 'ltTtsEnabled', 'ltTtsSpeed', 'ltTtsGender', 'ltTtsOriginalAudio', 'ltSubtitleLines', 'ltSubtitleTypewriter', 'ltSubtitleStyle', 'ltMuteTab', 'ltTopic', 'isCapturing', 'activeTabId']);
    ...
    // Sync per-language TTS voice (handled inside updateVoiceGroupVisibility)
    if (typeof updateVoiceGroupVisibility === 'function') {
      await updateVoiceGroupVisibility();
    }
...
async function toggleLiveTranslation() {
  ...
  if (!ltListening) {
    ...
    // The side panel is never an "invocation" surface, so activeTab is never
    // granted here. The background asks chrome.tabCapture on our behalf, which
    // succeeds wherever a host permission covers the page (x.com) or activeTab
    // was already granted for this tab.
    try {
      ...
      try {
        ...
        console.warn('⚠️ [Sidepanel] Content script injection failed or was restricted:', injectErr);
      }
      ...
      const startResponse = await sendMessage({
        ...
        ltEngine: document.getElementById('ltEngine').value
      });
      ...
      if (startResponse && startResponse.cause === 'stop_cooldown') {
        console.log('🎙️ [Sidepanel] Start ignored: it raced a just-finished stop.');
        setLiveStatus(false);
        return;
      }
      ...
      chrome.tabs.get(targetTabId, (capturedTab) => {
        if (capturedTab && capturedTab.url) window.currentCapturedTabUrl = capturedTab.url;
      });

      setLiveStatus(true);
      toast('success', 'Tab audio capture active');
    } catch (err) {
      console.error('🎙️ [Sidepanel] Failed to start capture:', err);
  ...
  if (message.action === 'lt_subtitle') {
    ...
    if (message.sequenceNumber !== undefined) {
      if (message.sequenceNumber < lastSubtitleSeq) {
        console.log(`🎙️ [SidePanel] Drop out-of-order/stale subtitle segment (seq: ${message.sequenceNumber} < last: ${lastSubtitleSeq})`);
        return;
      }
...
async function getSessionCaptions(sessionId) {
  ...
  try {
    ...
    console.warn('[SidePanel] Failed to get session captions:', err);
    return [];
  }
}
...
async function clearLiveCaptions() {
  ...
  if (selectedSession === 'current') {
    ...
    try {
      ...
      console.warn('[SidePanel] Failed to delete past session:', err);
...
async function renderHistoryPanel() {
  ...
  try {
    ...
    console.warn('[SidePanel] Failed to load ltSessionHistory:', err);
  }
...
async function loadCaptionHistoryFromStorage() {
  try {
    ...
    if (pendingSubtitleMessages.length > 0) {
      console.log('[Sidepanel] Flushing', pendingSubtitleMessages.length, 'queued subtitle message(s).');
